...
def vectorize_movie(movie: dict) -> np.ndarray:
    ...
    pop = min(movie.get('popularity', 0) / 1000.0, 1.0)  # Cap popularity at 1000 for scaling
    vote = movie.get('vote_average', 0) / 10.0           # 0-10 scale down to 0-1
    
    return np.array(genre_vector + [pop, vote], dtype=np.float32)
...
def aggregate_user_profile(user_interactions: list, movies_data: dict) -> np.ndarray:
    ...
    if not vectors:
        # 19 genres + 2 continuous = 21 dim vector
        return np.zeros(len(TMDB_GENRES) + 2, dtype=np.float32)
